...
export async function POST(request) {
  ...
  const validLocalFiles = localFiles
    .map((file) => ({
  ...
  try {
    const album = await loadAlbumAssets(series, slug);
    if (!album) {
      return NextResponse.json({ error: `Shared album not found for ${series}/${slug}` }, { status: 404 });
    }
    if (!album.assets.length) {
      return NextResponse.json({ error: `Shared album ${series}/${slug} has no imported assets.` }, { status: 400 });
    }
