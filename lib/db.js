...
function upsertSharedAlbum(db, { album_key, series, slug, title, year, race, cover_thumb_url, created_at, updated_at }) {
  db.prepare(`
    ...
    ON CONFLICT(album_key) DO UPDATE SET
      ...
      updated_at = excluded.updated_at
  `).run(album_key, series, slug, title, year ?? null, race ?? null, cover_thumb_url ?? null, created_at, updated_at);
}
...
function getSharedAlbumsBySeries(db, series) {
  ...
  `).all(series).map((r) => ({
    ...
    year: Number.isFinite(Number(r.year)) ? Number(r.year) : null,
    race: r.race || null,
    coverThumbUrl: r.cover_thumb_url || null,
...
function getSharedAlbumBySlug(db, { series, slug }) {
  ...
  return {
    ...
    race: row.race || null,
    coverThumbUrl: row.cover_thumb_url || null,
