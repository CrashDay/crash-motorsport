...
function initSchema(db) {
  ...
  ensureColumn(db, "map_pages", "geo_json", "TEXT");
  ensureColumn(db, "map_pages", "load_pins", "INTEGER NOT NULL DEFAULT 1");
}
  ...
  const rows = db.prepare(`
    SELECT
      ...
      (
        ...
        WHERE pa2.pin_id = p.pin_id
        ORDER BY pa2.sort_order DESC, pa2.added_at DESC, pa2.asset_id DESC
        LIMIT 1
      ) AS source_album_key,
      (
        SELECT sa.slug
...
function getSharedAlbumAssetsByAlbumKey(db, album_key) {
  ...
  `).all(album_key).map((r) => ({
    ...
    race: r.race || null,
    assignedAt: r.assigned_at,
    captureTime: r.capture_time || null,
  }));
}
...
function normalizeMapPageRow(row) {
  if (!row) return null;
  const centerLat = Number(row.center_lat);
  const centerLng = Number(row.center_lng);
  ...
  return {
    ...
    loadPins: Boolean(row.load_pins),
    createdAt: row.created_at,
    updatedAt: row.updated_at,
  };
}

function getMapPages(db) {
  return db.prepare(`
    SELECT track_id, title, center_lat, center_lng, zoom, geo_json, load_pins, created_at, updated_at
    FROM map_pages
    ORDER BY title ASC
  `).all().map(normalizeMapPageRow);
}

function getMapPage(db, track_id) {
  return normalizeMapPageRow(db.prepare(`
    SELECT track_id, title, center_lat, center_lng, zoom, geo_json, load_pins, created_at, updated_at
    FROM map_pages
    WHERE track_id = ?
  `).get(track_id));
...
function upsertMapPage(db, { track_id, title, center_lat, center_lng, zoom, geo_json, load_pins, created_at, updated_at }) {
  db.prepare(`
    ...
    ON CONFLICT(track_id) DO UPDATE SET
      ...
      updated_at = excluded.updated_at
  `).run(track_id, title, center_lat, center_lng, zoom, geo_json, load_pins ? 1 : 0, created_at, updated_at);
}
