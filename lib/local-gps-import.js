...
export async function runLocalGpsImport({ series, slug, trackId = DEFAULT_TRACK_ID, localFiles, dryRun = false, metadataSource = "browser" }) {
  ...
  const projector = getProjectorForTrack(normalizedTrackId);
  if (!projector) {
    throw new Error(`Track projector is not configured for ${normalizedTrackId}`);
  }
