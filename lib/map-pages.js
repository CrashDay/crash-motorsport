...
import { getDb, getMapPage, getMapPages, upsertMapPage } from "@/lib/db";
...
async function loadDatabaseMapPage(trackId) {
  ...
  return getMapPage(getDb(), trackId);
}
...
export async function saveMapPage(input) {
  ...
  upsertMapPage(getDb(), {
    ...
    zoom: input.zoom,
    geo_json: input.geoJsonText || null,
    load_pins: input.loadPins,
    created_at: now,
    updated_at: now,
  });
}
