...
import { parseJpegMetadata } from "@/lib/browser-exif-gps";
...
export async function readServerPhotoMetadata(buffer, fileName) {
  ...
  const parsedJpeg = parseJpegMetadata(
    buffer.buffer.slice(buffer.byteOffset, buffer.byteOffset + buffer.byteLength)
  );

  return {
    fileName: String(fileName || "").trim(),
    captureTime: parsedJpeg?.captureTime || parsedXmp.captureTime || "",
    gps: parsedJpeg?.gps || parsedXmp.gps || null,
    unsupported: false,
    debug: {
      ...
      exifGpsDebug: parsedJpeg?.debug?.exifGpsDebug || null,
