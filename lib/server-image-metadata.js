...
function parseXmpPacket(xmp) {
  ...
  const lat = parseXmpCoordinate(extractXmpValue(xmp, ["exif:GPSLatitude", "exifEX:GPSLatitude"]));
  const lon = parseXmpCoordinate(extractXmpValue(xmp, ["exif:GPSLongitude", "exifEX:GPSLongitude"]));
  return {
    captureTime,
...
export async function readServerPhotoMetadata(buffer, fileName) {
  ...
  const parsedXmp = xmp ? parseXmpPacket(xmp) : { captureTime: "", gps: null };
