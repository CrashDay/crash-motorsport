...
function getProjectorForTrack(trackId) {
  ...
  if (projectorCache.has(normalizedTrackId)) return projectorCache.get(normalizedTrackId);

  const filePath = path.join(process.cwd(), "public", "maps", `${normalizedTrackId}.geojson`);
  ...
  projectorCache.set(normalizedTrackId, projector);
  return projector;
}
